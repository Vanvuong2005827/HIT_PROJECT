...
public class ChapterScreen extends JFrame {
    ...
    private void nextEvent(MouseEvent evt) {
        ...
        chapterPrevious.setForeground(Color.BLACK);
        chapterNext.setForeground((chapterLastest + 1 >= chapters.size() - 1) ? Color.gray : Color.BLACK);
        if (chapterLastest == chapters.size() - 1) return;
        chapterLastest++;
        ...
        indexChapter = chapterLastest;
        processData();
    }

    private void previousEvent(MouseEvent evt) {
        int chapterLastest = bookService.getLastReadIndexChapter(curBook.getId());
        chapterNext.setForeground(Color.BLACK);
        chapterPrevious.setForeground((chapterLastest - 1 <= 0) ? Color.gray : Color.BLACK);
